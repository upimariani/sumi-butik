...
class mAnalisis extends CI_Model
{
	...
	{
		$data['all'] = $this->db->query("SELECT COUNT(po.id_po) as frequency, DATEDIFF('2023-06-30', MAX(tgl_po)) as recency, SUM(total_bayar) as monetary, po.id_pelanggan, nm_pel, MAX(tgl_po) as akhir_transaksi, MIN(tgl_po) as awal_transaksi, level_member FROM `po` JOIN pelanggan ON pelanggan.id_pelanggan=po.id_pelanggan WHERE MONTH(tgl_po) <= 6 AND MONTH(tgl_po) > 3 AND YEAR(tgl_po)='2023' GROUP BY po.id_pelanggan")->result();
